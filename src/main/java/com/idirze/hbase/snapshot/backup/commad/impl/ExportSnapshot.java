...
import com.idirze.hbase.snapshot.backup.admin.FileLink;
...
public class ExportSnapshot extends Configured implements Tool {
    ...
    private static class ExportMapper extends Mapper<BytesWritable, NullWritable,
        ...
        private String getFileChecksum(final FileSystem fs, final Path path) {
            try {
                switch (fs.getScheme()) {
                    case "file":
                        return HadoopChecksum.calculate(path.toUri().getPath());
                    case "hdfs":
                    default:
                        String algo = fs.getFileChecksum(path).getAlgorithmName();
                        return fs.getFileChecksum(path).toString().replace(algo + ":", "");
                }
            } catch (IOException e) {
        ...
        private boolean sameFile(final FileStatus inputStat, final FileStatus outputStat) {
            ...
            String inChecksum = getFileChecksum(inputFs, inputStat.getPath());
            if (inChecksum == null) return false;

            String outChecksum = getFileChecksum(outputFs, outputStat.getPath());
